...
# LED Setup (ESP32-C6 Super Mini uses Pin 8 for onboard WS2812)
from machine import Pin
try:
    from neopixel import NeoPixel
    led_np = NeoPixel(Pin(8), 1)
except Exception:
    led_np = None

def set_led(color):
    """Set the onboard RGB LED color (R, G, B) 0-255"""
    if led_np:
        try:
            led_np[0] = color
            led_np.write()
...
COLOR_GREEN = (0, 100, 0)
COLOR_BLUE = (0, 0, 100)
COLOR_YELLOW = (100, 80, 0) # Orange/Yellow
COLOR_CYAN = (0, 100, 100)
...
def main():
    ...
    # Original MQTT Path (Restore Handled Error Flow)
    ...
    # MAGIC CODE: Create UDP socket if stream IP is configured
    stream_sock = None
    stream_dest = None
    stream_seq = 0
    stream_buf = None
    
    # Use getattr for safety in MicroPython to avoid silent crashes
    stream_dest_ip = getattr(config, 'STREAM_DEST_IP', None)
    
    if stream_dest_ip:
        print(f"🚀 AUTO-STREAM ENABLED: Sending raw CSI to {stream_dest_ip}:5001")
        try:
            stream_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            stream_dest = (stream_dest_ip, 5001)
            stream_buf = bytearray(7 + 128)
            stream_buf[0] = 0x53
            stream_buf[1] = 0x43
            # Determine chip family code
            stream_buf[2] = 6 if g_state.chip_type == 'C6' else (5 if g_state.chip_type == 'C5' else (3 if g_state.chip_type == 'S3' else 1))
            stream_buf[3] = 0x01 if not g_state.needs_cv_normalization else 0x00
            stream_buf[5] = 64
            stream_buf[6] = 0
            set_led(COLOR_GREEN)
        except Exception as e:
            print(f"⚠️  STREAM ENGINE ERROR: {e}")
    else:
        print("⚠️  STREAM ENGINE DISABLED: No STREAM_DEST_IP found in config")

    # Force garbage collection before main loop
    gc.collect()
    print(f"Memory Ready: {gc.mem_free()} bytes free")
    print("------------------------------------------------------------")
    ...
    try:
        while True:
            ...
            if frame:
                ...
                # MAGIC CODE: Send packet via UDP if stream is enabled
                if stream_sock is not None:
                    try:
                        stream_buf[4] = stream_seq
                        stream_buf[7:] = csi_data
                        stream_sock.sendto(stream_buf, stream_dest)
                        stream_seq = (stream_seq + 1) & 0xFF
                        
                        # Visible heartbeat for debugging (triggers every 100 packets)
                        if stream_seq % 100 == 0:
                            print(f"📡 3D STREAMING ACTIVE: Sent {stream_seq} packets to {stream_dest[0]}")
                    except Exception:
                        pass
